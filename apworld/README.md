...
`template.yaml` is the player options template that matches this world version. It is generated by
Archipelago, not edited by hand: after changing options, package the world into an Archipelago checkout's
`custom_worlds/` (see Tests below) and regenerate it from that checkout:

```
python -c "import Options; Options.generate_yaml_templates('templates', False)"
cp "templates/Star Fox 64 (Starship).yaml" <Starship>/apworld/template.yaml
```
