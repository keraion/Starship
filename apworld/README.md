...
The player options template is not kept in the repo: Archipelago generates it from `options.py`. CI builds it
on every push (artifact `star_fox_64_ss-template.yaml`), and a tag push attaches it to the draft release as
`Starship_<tag>_Default_Template.yaml`. To make one locally, package the world into an Archipelago checkout's
`custom_worlds/` (see Tests below) and run, from that checkout:

```
python -c "import Options; Options.generate_yaml_templates('templates', False)"
```

The launcher's "Generate Template Options" writes the same file.

## Releases

Push a tag (for example `git tag 0.2.0 && git push origin 0.2.0`). Once every build and test job passes, the
`release` job in `.github/workflows/main.yml` creates a draft GitHub release for that tag with the apworld, the
default template and the Linux / Mac / Windows zips attached. Review it on GitHub and publish it by hand.
